    ...
    public function getTabs(): array
    {
        return [
            ...
            'This week' => Tab::make()
                ...
                ->badge(User::query()->where('created_at','>=', Carbon::now()->subWeek())->count()),
            'This month' => Tab::make()
                ->modifyQueryUsing(fn (Builder $query) => $query->where('created_at','>=', carbon::now()->submonth()))
                ->badge(User::query()->where('created_at','>=', Carbon::now()->submonth())->count()),
            'This year' => Tab::make()
                ->modifyQueryUsing(fn (Builder $query) => $query->where('created_at','>=', carbon::now()->subyear()))
                ->badge(User::query()->where('created_at','>=', Carbon::now()->subyear())->count()),
        ];
    }
}
